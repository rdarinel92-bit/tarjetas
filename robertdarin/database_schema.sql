-- ══════════════════════════════════════════════════════════════════════════════
-- SQL MAESTRO V1.0 — ROBERT DARIN FINTECH
...
CREATE TABLE IF NOT EXISTS prestamos (
  ...
  estado TEXT DEFAULT 'activo', -- activo, pagado, vencido, mora, cancelado, liquidado, reestructurado, legal
  proposito TEXT,
  garantia TEXT,
...
-- Habilitar RLS en tablas de configuración del sistema (V10.35)
ALTER TABLE configuracion_global ENABLE ROW LEVEL SECURITY;
ALTER TABLE temas_app ENABLE ROW LEVEL SECURITY;
ALTER TABLE fondos_pantalla ENABLE ROW LEVEL SECURITY;
...
BEGIN
    ...
    -- Configuración APIs: solo superadmin puede modificar
...
-- ══════════════════════════════════════════════════════════════════════════════
-- SECCIÓN 46: FUNCIÓN RPC PARA CREAR EMPLEADOS (V10.35)
-- Bypass RLS para permitir que admins creen empleados sin problemas
-- ══════════════════════════════════════════════════════════════════════════════

-- Función para crear empleado con todos los registros necesarios
CREATE OR REPLACE FUNCTION crear_empleado_completo(
    p_auth_user_id UUID,        -- ID del usuario de auth.users
    p_email TEXT,
...
COMMENT ON FUNCTION crear_empleado_completo IS 'Crea un empleado completo con usuario, rol y registro de auditoría. Requiere ser admin o superior. V10.35';
...
-- ══════════════════════════════════════════════════════════════════════════════
-- SECCIÓN 47: MÓDULO CLIMAS PROFESIONAL - MEJORAS COMPLETAS
-- ══════════════════════════════════════════════════════════════════════════════
...
-- Productos/Refacciones (catálogo completo)
CREATE TABLE IF NOT EXISTS climas_productos (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    negocio_id UUID REFERENCES negocios(id) ON DELETE CASCADE,
    codigo TEXT UNIQUE,
    nombre TEXT NOT NULL,
    descripcion TEXT,
    categoria TEXT, -- equipo, refaccion, consumible, herramienta
    subcategoria TEXT,
    marca TEXT,
    modelo TEXT,
    unidad TEXT DEFAULT 'pieza', -- pieza, metro, litro, kg
    precio_compra DECIMAL(14,2) DEFAULT 0,
...
CREATE POLICY "climas_cotizaciones_access" ON climas_cotizaciones FOR ALL USING (auth.role() = 'authenticated');
...
-- ══════════════════════════════════════════════════════════════════════════════
-- FIN SECCIÓN 47 - CLIMAS PROFESIONAL
-- ══════════════════════════════════════════════════════════════════════════════

-- ══════════════════════════════════════════════════════════════════════════════
-- SECCIÓN 48: ROLES Y PERMISOS PARA MÓDULO CLIMAS
-- ══════════════════════════════════════════════════════════════════════════════

-- 1. Crear roles específicos para Climas
INSERT INTO roles (id, nombre, descripcion) VALUES 
...
AND p.clave_permiso IN ('ver_climas_dashboard', 'climas_tecnico_ver_ordenes', 'climas_tecnico_ejecutar_ordenes',
    'climas_tecnico_checklist', 'climas_tecnico_fotos', 'climas_tecnico_firmas', 
    'climas_tecnico_materiales', 'climas_tecnico_ver_comisiones', 'ver_dashboard')
ON CONFLICT (rol_id, permiso_id) DO NOTHING;

-- 4. Asignar permisos al rol ADMIN_CLIMAS (todos los permisos de climas)
INSERT INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permisos p
WHERE r.nombre = 'ADMIN_CLIMAS'
AND (p.clave_permiso LIKE 'climas%' OR p.clave_permiso LIKE 'ver_climas%' OR p.clave_permiso LIKE 'gestionar_climas%' OR p.clave_permiso = 'ver_dashboard')
ON CONFLICT (rol_id, permiso_id) DO NOTHING;
...
AND p.clave_permiso IN ('ver_climas_dashboard', 'climas_cliente_solicitar', 'climas_cliente_ver_equipos',
    'climas_cliente_ver_historial', 'climas_cliente_ver_garantias', 'climas_cliente_mensajes', 
    'climas_cliente_calificar', 'ver_dashboard')
ON CONFLICT (rol_id, permiso_id) DO NOTHING;
...
-- ══════════════════════════════════════════════════════════════════════════════
-- FIN SECCIÓN 48 - ROLES Y PERMISOS CLIMAS
-- ══════════════════════════════════════════════════════════════════════════════

-- ══════════════════════════════════════════════════════════════════════════════
-- SECCIÓN 49: ROLES ESPECÍFICOS PARA PERSONAL DE MÓDULOS (V10.55)
-- Roles con nombres en minúsculas para compatibilidad con el sistema de auth
-- ══════════════════════════════════════════════════════════════════════════════

-- 1. Roles para personal de módulos (nombres en minúsculas)
INSERT INTO roles (id, nombre, descripcion) VALUES 
    -- Técnicos y personal de campo
    (gen_random_uuid(), 'tecnico_climas', 'Técnico de aires acondicionados - Ve dashboardTecnicoClimas'),
    (gen_random_uuid(), 'repartidor_purificadora', 'Repartidor de agua purificada - Ve dashboardRepartidorPurificadora'),
    -- Vendedores
    (gen_random_uuid(), 'vendedora_nice', 'Vendedora/Consultora NICE Joyería MLM - Ve dashboardVendedoraNice'),
    (gen_random_uuid(), 'vendedor_ventas', 'Vendedor del módulo de ventas/catálogo - Ve dashboardVendedorVentas'),
    -- Colaboradores
    (gen_random_uuid(), 'colaborador', 'Colaborador/Socio del negocio - Ve dashboardColaborador'),
    (gen_random_uuid(), 'inversionista', 'Inversionista con participación - Ve dashboardColaborador')
ON CONFLICT (nombre) DO UPDATE SET descripcion = EXCLUDED.descripcion;

-- 2. Asignar permisos básicos a técnicos de climas
INSERT INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permisos p
WHERE r.nombre = 'tecnico_climas'
...
-- 3. Asignar permisos básicos a repartidores de purificadora
INSERT INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permisos p
...
-- 4. Asignar permisos básicos a vendedoras NICE
INSERT INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permisos p
...
-- 5. Asignar permisos básicos a vendedores de ventas
INSERT INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permisos p
...
-- 6. Asignar permisos básicos a colaboradores
INSERT INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permisos p
WHERE r.nombre IN ('colaborador', 'inversionista')
AND p.clave_permiso IN (
    'ver_dashboard'
)
ON CONFLICT (rol_id, permiso_id) DO NOTHING;
...
-- ══════════════════════════════════════════════════════════════════════════════
-- FIN SECCIÓN 49 - ROLES ESPECÍFICOS PARA PERSONAL
-- ══════════════════════════════════════════════════════════════════════════════

-- ══════════════════════════════════════════════════════════════════════════════
-- SECCIÓN 50 - TARJETAS TEMPLATES (Para Centro de Control)
-- ══════════════════════════════════════════════════════════════════════════════

-- Tabla para plantillas de tarjetas de servicio
...
-- Política de acceso
CREATE POLICY "tarjetas_templates_select" ON tarjetas_templates FOR SELECT USING (true);
CREATE POLICY "tarjetas_templates_admin" ON tarjetas_templates FOR ALL USING (auth.role() = 'authenticated');

-- Índice para búsqueda
CREATE INDEX IF NOT EXISTS idx_tarjetas_templates_modulo ON tarjetas_templates(modulo);
CREATE INDEX IF NOT EXISTS idx_tarjetas_templates_activo ON tarjetas_templates(activo);

-- Insertar templates predeterminados
INSERT INTO tarjetas_templates (nombre, descripcion, modulo, icono, color, orden) VALUES
    ('Link de Pago', 'Genera links de pago para compartir con clientes', 'pagos', 'link', 'blue', 1),
    ('Cobro Recurrente', 'Para pagos mensuales o periódicos', 'pagos', 'repeat', 'purple', 2),
    ('Factura Digital', 'Genera facturas electrónicas', 'facturacion', 'receipt', 'green', 3),
    ('Promoción', 'Crea ofertas y descuentos', 'marketing', 'local_offer', 'orange', 4),
    ('Recordatorio', 'Envía recordatorios automáticos', 'notificaciones', 'schedule', 'cyan', 5),
    ('QR Negocio', 'Código QR para tu negocio', 'qr', 'qr_code', 'indigo', 6)
ON CONFLICT DO NOTHING;

-- ══════════════════════════════════════════════════════════════════════════════
-- FIN SECCIÓN 50 - TARJETAS TEMPLATES
-- ══════════════════════════════════════════════════════════════════════════════

-- ══════════════════════════════════════════════════════════════════════════════
-- SECCIÓN 51: CHAT WEB PARA TARJETAS DIGITALES (V10.53)
-- Sistema de mensajería web para visitantes de tarjetas QR
-- ══════════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════════
-- FIN SECCIÓN 51 - CHAT WEB TARJETAS
-- ══════════════════════════════════════════════════════════════════════════════

-- ══════════════════════════════════════════════════════════════════════════════
-- SECCIÓN 52: SOLICITUDES WEB DE TARJETAS QR (V10.55)
-- Formulario de contacto desde la página web de tarjetas QR
-- ══════════════════════════════════════════════════════════════════════════════

-- Tabla para solicitudes de contacto desde tarjetas QR de servicio
CREATE TABLE IF NOT EXISTS tarjetas_servicio_solicitudes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tarjeta_id UUID,
    negocio_id UUID,
    modulo TEXT,
...
-- Habilitar RLS
ALTER TABLE tarjetas_servicio_solicitudes ENABLE ROW LEVEL SECURITY;

-- Política para anónimos (pueden insertar desde la web)
...
-- Política para usuarios autenticados (pueden ver y gestionar)
DROP POLICY IF EXISTS tarjetas_servicio_solicitudes_auth ON tarjetas_servicio_solicitudes;
CREATE POLICY tarjetas_servicio_solicitudes_auth ON tarjetas_servicio_solicitudes 
    FOR ALL USING (auth.role() = 'authenticated');

-- Índices
CREATE INDEX IF NOT EXISTS idx_tss_tarjeta ON tarjetas_servicio_solicitudes(tarjeta_id);
CREATE INDEX IF NOT EXISTS idx_tss_negocio ON tarjetas_servicio_solicitudes(negocio_id);
CREATE INDEX IF NOT EXISTS idx_tss_estado ON tarjetas_servicio_solicitudes(estado);
CREATE INDEX IF NOT EXISTS idx_tss_created ON tarjetas_servicio_solicitudes(created_at DESC);

COMMENT ON TABLE tarjetas_servicio_solicitudes IS 'Solicitudes de contacto desde tarjetas QR de servicio web';

-- ══════════════════════════════════════════════════════════════════════════════
-- FIN SECCIÓN 52 - SOLICITUDES WEB TARJETAS QR
-- ══════════════════════════════════════════════════════════════════════════════

-- ══════════════════════════════════════════════════════════════════════════════
-- FIN DEL SCHEMA - SQL MAESTRO V1.0 COMPLETO
