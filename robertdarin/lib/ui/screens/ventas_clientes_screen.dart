...
/// ═══════════════════════════════════════════════════════════════════════════════
/// CLIENTES DEL MÓDULO VENTAS - CRUD Completo
/// ═══════════════════════════════════════════════════════════════════════════════
class VentasClientesScreen extends StatefulWidget {
  const VentasClientesScreen({super.key});
  @override
  State<VentasClientesScreen> createState() => _VentasClientesScreenState();
}

class _VentasClientesScreenState extends State<VentasClientesScreen> {
  bool _isLoading = true;
  List<VentasClienteModel> _clientes = [];
  String _searchQuery = '';
  ...
  Future<void> _cargarClientes() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _clientes = (res as List).map((e) => VentasClienteModel.fromMap(e)).toList();
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Widget _buildClienteCard(VentasClienteModel cliente) {
    return Container(
      ...
      child: ListTile(
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: _getColorTipo(cliente.tipo).withOpacity(0.2),
                    borderRadius: BorderRadius.circular(6),
                  ),
                  child: Text(
                    cliente.tipo.toUpperCase(),
                    style: TextStyle(color: _getColorTipo(cliente.tipo), fontSize: 9),
                  ),
                ),
                if (cliente.limiteCredito > 0) ...[
                  const SizedBox(width: 8),
                  Icon(Icons.credit_card, size: 12, color: Colors.white.withOpacity(0.5)),
                  Text(
                    ' \$${cliente.limiteCredito.toStringAsFixed(0)}',
        ...
        trailing: PopupMenuButton<String>(
          ...
          onSelected: (value) {
            ...
            if (value == 'detail') {
              Navigator.pushNamed(context, '/ventas/cliente/detalle', arguments: cliente.id);
            }
          },
          itemBuilder: (context) => [
            const PopupMenuItem(value: 'detail', child: Row(children: [Icon(Icons.visibility, color: Color(0xFF8B5CF6), size: 18), SizedBox(width: 8), Text('Ver Detalle', style: TextStyle(color: Colors.white))])),
        ...
        onTap: () => Navigator.pushNamed(context, '/ventas/cliente/detalle', arguments: cliente.id),
      ),
    );
  }
  ...
  Color _getColorTipo(String tipo) {
    switch (tipo) {
      case 'minorista': return const Color(0xFF10B981);
      case 'mayorista': return const Color(0xFF8B5CF6);
      case 'distribuidor': return const Color(0xFFF59E0B);
      default: return Colors.white54;
    }
  }

  Future<void> _mostrarFormulario({VentasClienteModel? cliente}) async {
    ...
    final limiteCreditoController = TextEditingController(text: cliente?.limiteCredito.toString() ?? '0');
    final passwordController = TextEditingController(); // V10.22 Auth
    String tipoCliente = cliente?.tipo ?? 'minorista';
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                _buildTextField(limiteCreditoController, 'Límite de crédito', Icons.credit_card, keyboardType: TextInputType.number),
                const SizedBox(height: 16),
                const Text('Tipo de Cliente', style: TextStyle(color: Colors.white70, fontSize: 12)),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(child: _buildTipoBtn('Minorista', tipoCliente == 'minorista', () => setDialogState(() => tipoCliente = 'minorista'))),
                    const SizedBox(width: 4),
                    Expanded(child: _buildTipoBtn('Mayorista', tipoCliente == 'mayorista', () => setDialogState(() => tipoCliente = 'mayorista'))),
                    const SizedBox(width: 4),
                    Expanded(child: _buildTipoBtn('Distrib.', tipoCliente == 'distribuidor', () => setDialogState(() => tipoCliente = 'distribuidor'))),
                  ],
                ),
            ...
            ElevatedButton(
              onPressed: guardando ? null : () async {
                ...
                final data = {
                  ...
                  'limite_credito': double.tryParse(limiteCreditoController.text) ?? 0,
                  'tipo': tipoCliente,
                ...
                try {
                  if (cliente == null) {
                    await AppSupabase.client.from('ventas_clientes').insert(data);
                  } else {
                    await AppSupabase.client.from('ventas_clientes').update(data).eq('id', cliente.id);
                  }
  ...
  Widget _buildTipoBtn(String label, bool selected, VoidCallback onTap) {
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected ? const Color(0xFF8B5CF6).withOpacity(0.2) : const Color(0xFF0D0D14),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: selected ? const Color(0xFF8B5CF6) : Colors.transparent),
        ),
        child: Center(
          child: Text(label, style: TextStyle(color: selected ? const Color(0xFF8B5CF6) : Colors.white54, fontSize: 11)),
        ),
      ),
    );
  }

  Future<void> _confirmarEliminar(VentasClienteModel cliente) async {
    ...
    if (confirm == true) {
      try {
        await AppSupabase.client.from('ventas_clientes').delete().eq('id', cliente.id);
        _cargarClientes();
      } catch (e) {
